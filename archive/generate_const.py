#!/usr/bin/env python3
"""Script to generate updated const.py with all register mappings."""

import json

# Load the fixed registers file
with open('registers_fixed.json') as f:
    data = json.load(f)

# List of already implemented registers (from current const.py)
...
# Generate new register map by adding all registers from the JSON file
new_register_map = {}
...
for reg in data['registers']:
    ...
    # Skip if already implemented (we'll keep existing ones)
    if name not in implemented_registers:
        new_register_map[name] = address

# Sort by address for better organization
sorted_new_registers = sorted(new_register_map.items(), key=lambda x: x[1])

print("""
# New registers to add to REGISTER_MAP:
print("=" * 80)
for name, addr in sorted_new_registers:
    print(f'    "{name}": {addr},')
print("=" * 80)
print(f"\nTotal new registers: {len(sorted_new_registers)}")
""")

# Generate the complete updated const.py file
const_content = '''"""Constants for the Midnite Solar integration."""
...
# Add all registers sorted by address
all_registers = {**implemented_registers, **new_register_map}
sorted_all_registers = sorted(all_registers.items(), key=lambda x: x[1])

for name, addr in sorted_all_registers:
    const_content += f'    "{name}": {addr},\n'
...
print("\nGenerated const.py content (partial shown):")
print(const_content[:500])
print("...")
print(f"\nTotal lines: {len(const_content.splitlines())}")
