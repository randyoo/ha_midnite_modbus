#!/usr/bin/env python3
"""
Test the statistical filtering fix specifically.
This test bypasses rate-of-change detection to focus on the z-score threshold change.
"""
...
def validate_temperature_statistical_only(value, sensor_name, recent_readings):
    ...
    Bypasses range and rate-of-change checks to focus on z-score threshold.
...
def test_statistical_filtering():
    """Test the statistical filtering with different z-score thresholds."""
    
    print("Testing temperature sensor statistical filtering...")
    print("=" * 60)
    
    # Test 1: Scenario from the bug report
    print("\n1. Testing scenario from bug report:")
    print("   Baseline: 20 readings at 55.06°C (std dev ~0.11°C)")
    print("   Problematic reading: 46.5°C")
    
    recent_readings = [55.06] * 20
    mean_temp = sum(recent_readings) / len(recent_readings)
    variance = sum((x - mean_temp) ** 2 for x in recent_readings) / len(recent_readings)
    ...
    if z_score_old_threshold > 4.0:
        print(f"     → Would be REJECTED (z-score > 4.0)")
    else:
        print(f"     → Would be ACCEPTED (z-score <= 4.0)")
    
    print(f"   - New threshold (z > 6.0): z-score={z_score_new_threshold:.2f}")
    if z_score_new_threshold > 6.0:
        print(f"     → Would be REJECTED (z-score > 6.0)")
    else:
        print(f"     → Would be ACCEPTED (z-score <= 6.0)")
    ...
    print("   Baseline: 20 readings at 55.06°C")
    ...
    test_cases = [
        (46.5, "Legitimate drop after charge"),
        (50.0, "Moderate cooling"),
        (58.0, "Slight warming"),
        (20.0, "Sensor error (extreme)"),
    ]
