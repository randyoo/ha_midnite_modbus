...
class TestStatusRoll:
    ...
    def test_the_value_is_the_low_twelve_bits(self, entry):
        assert (
            one(StatusRollSensor, entry, "status", "STATUSROLL", 0x0001).native_value
            == 1
        )
    ...
    def test_the_biggest_value_the_field_holds(self, entry):
        assert (
            one(StatusRollSensor, entry, "status", "STATUSROLL", 0xFFFF).native_value
            == 4095
        )

    def test_no_reading_is_unknown(self, entry):
        assert (
            StatusRollSensor(FakeCoordinator(Hass(), FakeApi(), {}), entry).native_value
            is None
        )
...
class TestTenthsOfAVoltOrAmp:
    ...
    def test_a_discharge_reads_negative(self, entry):
        ...
        """
        assert one(
            BatteryCurrentSensor, entry, "status", "IBATT_DISPLAY_S", 65336
        ).native_value == pytest.approx(-20.0)
    ...
    def test_a_large_battery_current_is_not_invented_away(self, entry):
        ...
        """
        assert one(
            BatteryCurrentSensor, entry, "status", "IBATT_DISPLAY_S", 2400
        ).native_value == pytest.approx(240.0)

    def test_pv_input_current_reads_negative(self, entry):
        """4121 is the same two's-complement tenths as 4117."""
        assert one(
            PVInputCurrentSensor, entry, "status", "PV_INPUT_CURRENT", 65336
        ).native_value == pytest.approx(-20.0)

    def test_a_large_pv_current_is_not_invented_away(self, entry):
        assert one(
            PVInputCurrentSensor, entry, "status", "PV_INPUT_CURRENT", 1200
        ).native_value == pytest.approx(120.0)
    ...
    def test_no_current_is_dropped_for_a_ceiling_the_map_does_not_give(self, entry):
        ...
        """
        assert one(
            BatteryCurrentSensor, entry, "status", "IBATT_DISPLAY_S", 30000
        ).native_value == pytest.approx(3000.0)

    def test_a_temperature_below_zero_reads_negative(self, entry):
        """Map: "([4132] /10) °C"; the Classic must be able to say it is freezing."""
        assert one(
            BatteryTemperatureSensor, entry, "temperatures", "BATT_TEMPERATURE", 0xFF9C
        ).native_value == pytest.approx(-10.0)

    def test_the_two_internal_temperatures_read_the_same_way(self, entry):
        """4133 and 4134: "([4nnn] /10) °C"."""
        assert one(
            FETTemperatureSensor, entry, "temperatures", "FET_TEMPERATURE", 456
        ).native_value == pytest.approx(45.6)
        assert one(
            PCBTemperatureSensor, entry, "temperatures", "PCB_TEMPERATURE", 321
        ).native_value == pytest.approx(32.1)
...
class TestPlainCounts:
    ...
    def test_watts_are_watts(self, entry):
        """4119: "[4119] Watts"."""
        assert (
            one(PowerWattsSensor, entry, "status", "WATTS", 1234).native_value == 1234
        )
    ...
    def test_daily_amp_hours_are_amp_hours(self, entry):
        ...
        assert (
            one(
                DailyAmpHoursSensor, entry, "energy", "AMP_HOURS_DAILY", 42
            ).native_value
            == 42
        )
    ...
    def test_the_logging_interval_is_in_seconds(self, entry):
        ...
        assert (
            one(
                LoggingIntervalSensor, entry, "settings", "MINUTE_LOG_INTERVAL_SEC", 300
            ).native_value
            == 300
        )
    ...
    def test_the_modbus_port_is_the_port(self, entry):
        ...
        assert (
            one(
                ModbusPortSensor, entry, "settings", "MODBUS_PORT_REGISTER", 502
            ).native_value
            == 502
        )
    ...
    def test_the_match_point_is_a_step_number(self, entry):
        ...
        assert (
            one(
                MatchPointShadowSensor, entry, "status", "MATCH_POINT_SHADOW", 7
            ).native_value
            == 7
        )
...
class TestEnergyTotals:
    ...
    def test_lifetime_energy_is_tenths_of_a_kilowatt_hour(self, entry):
        ...
        """
        values = {
            REGISTER_MAP["LIFETIME_KW_HOURS_1"]: 0x0001,
            REGISTER_MAP["LIFETIME_KW_HOURS_1"] + 1: 0x0000,
        }
        assert sensor(
            LifetimeEnergySensor, entry, "energy", values
        ).native_value == pytest.approx(0.1)

    def test_the_high_word_counts(self, entry):
        values = {
            REGISTER_MAP["LIFETIME_KW_HOURS_1"]: 0xFFFF,
            REGISTER_MAP["LIFETIME_KW_HOURS_1"] + 1: 0x0001,
        }
        assert sensor(
            LifetimeEnergySensor, entry, "energy", values
        ).native_value == pytest.approx(13107.1)

    def test_lifetime_amp_hours_is_the_other_pair(self, entry):
        """4128 4129: "(([4129] << 16) + [4128]) Amp Hours" - no divisor (bench 202213)."""
        values = {
            REGISTER_MAP["LIFETIME_AMP_HOURS_1"]: 500,
            REGISTER_MAP["LIFETIME_AMP_HOURS_1"] + 1: 2,
        }
        assert (
            sensor(LifetimeAmpHoursSensor, entry, "energy", values).native_value
            == 2 * 65536 + 500
        )
    ...
    def test_daily_energy_is_tenths_of_a_kilowatt_hour(self, entry):
        ...
        assert (
            one(DailyEnergySensor, entry, "status", "KW_HOURS", 1234).native_value
            == 123.4
        )
...
class TestPackedBytes:
    ...
    def test_the_charge_stage_is_the_high_byte(self, entry):
        ...
        assert (
            one(
                ChargeStageSensor, entry, "status", "COMBO_CHARGE_STAGE", 0x0500
            ).native_value
            == "Float"
        )
    ...
    def test_the_charge_stage_is_not_confused_with_the_state(self, entry):
        ...
        assert (
            one(
                ChargeStageSensor, entry, "status", "COMBO_CHARGE_STAGE", 0x0003
            ).native_value
            == "Resting"
        )
    ...
    def test_the_device_type_is_the_low_byte_of_the_unit_id(self, entry):
        ...
        assert (
            one(
                DeviceTypeSensor, entry, "device_info", "UNIT_ID", (7 << 8) | 200
            ).native_value
            == "Classic 200"
        )
    ...
    def test_a_unit_type_the_table_does_not_have_is_reported_as_it_reads(self, entry):
        assert (
            one(DeviceTypeSensor, entry, "device_info", "UNIT_ID", 999).native_value
            == "Unknown (231)"
        )
...
class TestMACAddress:
    ...
    def test_the_highest_register_holds_the_first_two_octets(self):
        assert (
            self._mac(ConfigEntry(entry_id="e", title="t"), 0x0102, 0x0304, 0x00A1)
            == "00:a1:03:04:01:02"
        )
    ...
    def test_a_realistic_classic_address(self):
        ...
        assert (
            self._mac(ConfigEntry(entry_id="e", title="t"), 0x0101, 0x0202, 0x0303)
            == "03:03:02:02:01:01"
        )
    ...
    def test_a_missing_register_is_not_guessed_at(self, entry):
        coordinator = FakeCoordinator(
            Hass(),
            FakeApi(),
            {"device_info": {REGISTER_MAP["MAC_ADDRESS_PART_1"]: 0x0102}},
        )
