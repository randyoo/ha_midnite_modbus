...
class TestTemperatureNoiseFilter:
    ...
    def test_stable_readings_are_reported(self, hass, entry):
        sensor_obj, _ = sensor(
            hass, entry, BatteryTemperatureSensor, "temperatures", {4132: 250}
        )
        assert sensor_obj.native_value == pytest.approx(25.0)

    def test_negative_temperatures_are_signed(self, hass, entry):
        sensor_obj, _ = sensor(
            hass, entry, BatteryTemperatureSensor, "temperatures", {4132: 65535}
        )
        assert sensor_obj.native_value == pytest.approx(-0.1)

    @pytest.mark.parametrize(
        ("raw", "expected"),
        [(250, 25.0), (500, 50.0), (1500, 150.0), (65436, -10.0), (65535, -0.1)],
    )
    ...
    def test_a_reading_outside_physical_range_is_dropped_immediately(self, hass, entry):
        ...
        sensor_obj, _ = sensor(
            hass, entry, BatteryTemperatureSensor, "temperatures", {4132: 3000}
        )
    ...
    def test_a_warm_battery_is_still_reported(self, hass, entry):
        ...
        assert published[-1] == pytest.approx(36.0) or published[0] == pytest.approx(
            32.0
        )
...
class TestScalingFixes:
    ...
    def test_sliding_current_limit_is_whole_amps(self, hass, entry):
        ...
        sensor_obj, _ = sensor(
            hass, entry, SlidingCurrentLimitSensor, "settings", {4152: 12}
        )
    ...
    def test_lifetime_energy_is_tenths_of_kilowatt_hours(self, hass, entry):
        ...
        """
        sensor_obj, _ = sensor(
            hass, entry, LifetimeEnergySensor, "energy", {4126: 12345, 4127: 0}
        )
        assert sensor_obj.native_value == 1234.5

    def test_lifetime_energy_uses_the_high_word(self, hass, entry):
        sensor_obj, _ = sensor(
            hass, entry, LifetimeEnergySensor, "energy", {4126: 1, 4127: 1}
        )
    ...
    def test_the_bench_lifetime_energy(self, hass, entry):
        ...
        sensor_obj, _ = sensor(
            hass, entry, LifetimeEnergySensor, "energy", {4126: 0xAD5D, 4127: 0x0001}
        )
    ...
    def test_lifetime_amp_hours_is_amp_hours(self, hass, entry):
        ...
        sensor_obj, _ = sensor(
            hass, entry, LifetimeAmpHoursSensor, "energy", {4128: 4321, 4129: 2}
        )
...
class TestNetworkAddresses:
    ...
    def test_lower_register_supplies_the_first_octet(
        self, hass, entry, cls, low_address
    ):
    ...
    def test_the_octets_are_never_reversed(self, hass, entry):
        ...
        sensor_obj, _ = sensor(
            hass, entry, IPAddressSensor, "network", {20482: 0xA8C0, 20483: 0x1858}
        )
        assert sensor_obj.native_value == "192.168.88.24"
    ...
    def test_address_formatting_cases(self, hass, entry, low, high, expected):
        sensor_obj, _ = sensor(
            hass, entry, IPAddressSensor, "network", {20482: low, 20483: high}
        )
